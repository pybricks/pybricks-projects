...
# You can also use the TechnicHub and/or the ColorDistanceSensor instead.
hub = InventorHub()
...
while True:
    ...
    # Calculate reference position, which just grows linearly with time.
    reference = -max(watch.time() - PAUSE, 0) / 1000 * DRIVE_SPEED

    # Calculate duty cycle.
    duty = 0.018*rate + 19*angle + 0.45*(position - reference) + 0.16*speed
